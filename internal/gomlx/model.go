...
// NewGoMLXModel creates a new GoMLXModel from the given config.
// The model is lazily initialized on the first call to GenerateContent.
func NewGoMLXModel(cfg *GoMLXConfig) (*GoMLXModel, error) {
	return &GoMLXModel{cfg: cfg}, nil
}

// Init loads the model weights, tokenizer, and architecture from the GGUF file.
func (m *GoMLXModel) Init() error {
	m.initOnce.Do(func() {
		m.initErr = m.doInit()
	})
	return m.initErr
...
func (m *GoMLXModel) doInit() error {
...
func (m *GoMLXModel) GenerateContent(ctx context.Context, req *model.LLMRequest, stream bool) iter.Seq2[*model.LLMResponse, error] {
	return func(yield func(*model.LLMResponse, error) bool) {
		...
		if err := m.Init(); err != nil {
			yield(nil, fmt.Errorf("gomlx init: %w", err))
			return
		}
		...
		for chunk, err := range Generate(ctx, m.arch, m.tokenizer, prompt, params) {
			if err != nil {
				yield(nil, fmt.Errorf("gomlx generate: %w", err))
				return
			}
...
func (m *GoMLXModel) buildGenerateParams(req *model.LLMRequest) GenerateParams {
